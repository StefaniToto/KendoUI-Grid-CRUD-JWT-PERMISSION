...
var CheckDirective = /** @class */ (function () {
    function CheckDirective(treeView, zone) {
        ...
        this.subscriptions.add(this.treeView.childrenLoaded
            .pipe(operators_1.filter(function () { return _this.options.checkChildren; }), operators_1.tap(function (item) { return expandedItems.push(item); }), operators_1.switchMap(function () { return _this.zone.onStable.pipe(operators_1.take(1)); }))
            .subscribe(function () {
            var rootItemKey = expandedItems[0] && _this.itemKey(expandedItems[0].item);
            var checkChildren = utils_1.isPresent(rootItemKey) && _this.checkedKeys.some(function (key) { return key === rootItemKey; });
            if (checkChildren) {
                _this.checkChildren(expandedItems);
            }
            expandedItems = [];
        }));
    ...
    CheckDirective.prototype.checkNode = function (node, check) {
        var _this = this;
        if (!utils_1.isPresent(node.item.dataItem)) {
            return;
        }
        var key = this.itemKey(node.item);
        ...
        if (!utils_1.isPresent(key) || (isChecked && check) || this.treeView.isDisabledNode(node)) {
            return;
        }
        if (isChecked) {
    ...
    CheckDirective.prototype.checkChildren = function (items) {
        var _this = this;
        var pendingCheck = items.reduce(function (keys, item) {
            return keys = keys.concat(_this.getUncheckedChildrenKeys(item));
        }, []);
    ...
    CheckDirective.prototype.getUncheckedChildrenKeys = function (item) {
        var _this = this;
        return item.children.reduce(function (acc, item) {
            var dataItemPresent = utils_1.isPresent(item.dataItem);
            if (!dataItemPresent) {
                return acc;
            }
