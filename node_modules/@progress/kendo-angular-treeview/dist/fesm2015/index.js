...
class NavigationModel {
    ...
    registerItem(id, index, disabled, loadMoreButton = false) {
        const children = [];
        const level = this.ib.level(index);
        const parent = this.findParent(index);
        if (parent || level === 1) {
            const node = { id, children, index, parent, disabled, loadMoreButton };
...
/** @nocollapse */
LoadMoreButtonTemplateDirective.ctorParameters = () => [
    { type: TemplateRef, decorators: [{ type: Optional }] }
];
...
class CheckDirective {
    constructor(treeView, zone) {
        ...
        this.subscriptions.add(this.treeView.childrenLoaded
            ...
            const checkChildren = isPresent(rootItemKey) && this.checkedKeys.some(key => key === rootItemKey);
            if (checkChildren) {
    ...
    getUncheckedChildrenKeys(item) {
        return item.children.reduce((acc, item) => {
            const dataItemPresent = isPresent(item.dataItem);
            if (!dataItemPresent) {
                return acc;
            }
 ...
 */
const hasRelativeStackingContext = memoize(ɵ0$2);
/**
...
/** @nocollapse */
LoadMoreDirective.ctorParameters = () => [
    { type: TreeViewComponent }
];
